(function($) {
  ...
  function print_array(obj, opts) {
    ...
    for (var i = 0; i < Math.min(opts.max_array, obj.length); i++)
      result.push($.print(obj[i], $.extend({}, opts, { max_array: 3, max_string: 400 })));
  ...
  function print_object(obj, opts) {
    ...
    for (var k in obj) {
      if (obj.hasOwnProperty(k) && $.inArray(obj[k], seen) < 0) {
        seen.push(obj[k]);
        value = $.print(obj[k], $.extend({}, opts, { max_array: 6, max_string: 400, seen: seen }));
      } else
        value = "...";
      result.push(k + ": " + value);
    }
  ...
  $.print = function(obj, options) {
    var opts = $.extend({}, { max_array: 10, max_string: 10000 }, options);

    // Objects may define their own printed representation by defining a
    // `toString()` method.
    ...
    if (typeof obj == 'undefined')
      ...
      return print_object(obj, $.extend({}, options, { max_string: 20000 }));
